...
run_single_triplet_control() {
    ...
    # Monolithic
    sudo RUST_LOG=info ../target/release/main --exec-time ${exec_time} monolithic $app ${app_goal} > output
    python ../src/fix_output.py output $outdir/$app-monolithic

    # Dynamic
    sudo RUST_LOG=info ADAPT_TYPE=inaccuracy ADAPT_INST=0,1 DEV_TARGET=${dev_target} \
...
run_single_triplet_learning() {
    ...
    # Dynamic
    sudo RUST_LOG=info LEARNING_BASED=y CONF_TYPE=multi \
         ADAPT_TYPE=inaccuracy ADAPT_INST=0,1 DEV_TARGET=${dev_target} \
         ../target/release/main --exec-time ${exec_time} multimodule $app ${app_goal} > output
    python ../src/fix_output.py output $outdir/$app-dynamic
}
...
run_multi_control() {
    ...
    sudo RUST_LOG=info ADAPT_TYPE=inaccuracy ADAPT_INST=0,1,2 DEV_TARGET=$dev_target \
...
run_multi_learning() {
    ...
    sudo RUST_LOG=info LEARNING_BASED=y CONF_TYPE=multi \
         ADAPT_TYPE=inaccuracy ADAPT_INST=0,1,2 DEV_TARGET=$dev_target \
         ../target/release/main --exec-time 180 multi-app $app0 $goal0 $app1 $goal1 > output
    python ../src/fix_output.py output $outdir/${app0}-${app1}-dynamic
}
